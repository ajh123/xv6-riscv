...
e1000_tx_init()
{
  ...
  regs[E1000_TDBAL] = (uint64)(uint64) tx_ring;
...
e1000_rx_init()
{
  ...
  for (int i = 0; i < RX_RING_SIZE; i++) {
    ...
    if (!rx_mbufs[i])
      panic("e1000");
    rx_ring[i].addr = (uint64)(uint64) rx_mbufs[i]->head;
  }
  regs[E1000_RDBAL] = (uint64)(uint64) rx_ring;
  regs[E1000_RDH] = 0;
...
e1000_send(struct mbuf *m)
{
  ...
  tx_mbufs[desc_pos] = m;
  tail->addr = (uint64)(uint64) m->head;
  tail->length = m->len;
...
e1000_recv(void)
{
  ...
  while (1) {
    ...
    if (!rx_mbufs[desc_pos])
      panic("e1000");
    tail->addr = (uint64)(uint64) rx_mbufs[desc_pos]->head;
    tail->status = 0;
